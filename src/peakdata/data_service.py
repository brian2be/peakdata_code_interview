...
@app.route(
    "/fetch/<int:maxAgeDays>/<string:category>/<string:output>/<string:method>",
    methods=["POST"],
)
def fetchURL(maxAgeDays, category, output="html", method="GET"):
    try:
        ...
        theData = {
            "response": list(getData(urls, method, maxAgeDays, category, output))
            if len(urls) > 0
            else []
        }
...
def getData(urlList: list, method: str, maxAgeDays: int, category, output="xml"):
    ...
    urlData = {
        dbNormalizeURL(urlTuple): {
            "urlTuple": urlTuple,
            "urlKey": dbNormalizeURL(urlTuple),
        }
    ...
    existing = {
        data["urlKey"]: data
        for data in db.webpages.find(
            {
                "urlKey": {"$in": list(urlData.keys())},
                "format": output,
    ...
    print(
        "found %s entries already and will need to obtain an additional %s. Total Unique urls: %s"
    ...
    if len(urlKeysToObtain) > 0:
        ...
        processes = [
            multiprocessing.Process(
                target=processURLChunk,
                args=(
                    urlKeysToObtain[x : x + chunkLen],
                    urlData,
                    method,
                    output,
                    category,
            ...
            for x in range(0, len(urlKeysToObtain), chunkLen)
        ]
            ...
            for data in db.webpages.find(
                {
                    ...
                    "format": output,
                    "creation_date": {
                        "$gt": datetime.now() - timedelta(days=maxAgeDays)
                    },
                }
            )
        }
    ...
    for data in urlData:
        targetField = (
            "content_bz2"
            if "content_bz2" in urlData[data]
            and urlData[data]["content_bz2"] is not None
            else "content_raw_bz2"
        )
        if targetField in urlData[data]:
            urlData[data][targetField] = str(
                base64.b64encode(urlData[data][targetField])
            )
        else:
...
def updateDBEntry(result, urlTuple, nTries=2):
    ...
    try:
        db.webpages.replace_one(
            {"urlKey": dbNormalizeURL(urlTuple)}, result, upsert=True
        )
...
def tryNTimesToGetPage(
    ...
    with pymongo.MongoClient(MONGO_LOCATION) as client:
        ...
        try:
            ...
            print(
                "encountered exception on url %s: %s"
                % (dbNormalizeURL(urlTuple), traceback.format_exc())
            )
            return
...
def obtainPage(urlTuple: tuple, method: str, output: str, proxy: str):
    ...
    with requests.request(
        ...
        toReturn = {
            "download_duration_ms": timeDiffToNow(downloadStartTime),
            "content_bz2": parsedData,
            "size": len(data),
            "urlTuple": urlTuple,
            "format": output,
            "urlKey": dbNormalizeURL(urlTuple),
