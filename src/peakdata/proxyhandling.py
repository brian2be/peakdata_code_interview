...
class DBProxyHandler:
    ...
    def upload(self, proxyList):
        ...
        self.db.proxies.bulk_write(
            [
                ReplaceOne(
                    {"address": item},
                    {"address": item, "successful_job_completion": 0},
                    upsert=True,
                )
    ...
    def pick(self, n=1, nTries=3):
        ...
        try:
            ...
            proxies = list(
                self.db.proxies.find({"successful_job_completion": {"$gt": -30}}).limit(
                    min(10000, 1000 * n)
                )
            )
            for proxy in proxies:
                proxy["score"] = random.random() * min(
                    max(proxy["successful_job_completion"], -5), 5
                )

            chosenProxies = random.choices(
                population=proxies, weights=[proxy["score"] for proxy in proxies], k=n
            )
    ...
    def feedback(self, address, counter=1, nTries=3):
        ...
        try:
            ...
            self.db.proxies.update_one(
                ...
                {
                    "$set": {
                        "successful_job_completion": proxy.get(
                            "successful_job_completion", 0
                        )
                ...
                upsert=True,
            )  # allow max 15 plus points. if proxy goes offline, max 45 req will drop it
        except pymongo.errors.AutoReconnect:
