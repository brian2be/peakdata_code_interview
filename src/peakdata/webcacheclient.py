...
class WebCacheClient:  # add constructor to set webcache location programmatically. fall back to config if no explicit location provided
    ...
    def fetchURLs(self, urlList, category: str, output, method="GET", maxAgeDays=360):
        ...
        serviceURL = "http://%s/fetch/%s/%s/%s/%s" % (
            self.WEBCACHE_LOCATION,
            maxAgeDays,
            category,
            output,
            method,
        ...
        if any(
            "localhost" in url[0] or "127.0.0.1" in url[0] for url in filteredUrlList
        ):
            data = {}
            ...
            data = requests.post(
                serviceURL, {"urls": json.dumps(filteredUrlList)}
            ).json()
        ...
        if "error" in data:
            ...
            for pageData in data["response"]:
                target_field = (
                    "content_bz2"
                    if "content_bz2" in pageData and pageData["content_bz2"] is not None
                ...
                if target_field in pageData:
                    ...
                    if target_field != "content_bz2":
                        print(
                            "we could not parse url %s into %s"
                            % (dbNormalizeURL(pageData["urlTuple"]), output)
                        )
            ...
            return {
                urlItem: urlKeys.get(
                    dbNormalizeURL(urlItem),
                    {"url": urlItem, "content": None, "error": True},
                )
                for urlItem in urlList
            }
...
def dbNormalizeURL(urlItem):
    theUrl, theData = (
        (urlItem, {}) if type(urlItem) is str else (urlItem[0], json.loads(urlItem[1]))
    )
    try:
        lowerLinkFurl = furl.furl(
            theUrl.lower().strip().replace("https://", "http://")
        )  # consider http and https as EQUAL for the key
        lowerLinkFurl.path.normalize()
        lowerLinkFurl.query.params = sorted(
            [
                (par, lowerLinkFurl.query.params[par])
                for par in lowerLinkFurl.query.params
            ],
            key=lambda item: item[0],
        )
        lowerLinkFurl.path = (
            "%s/" % lowerLinkFurl.path
            if not str(lowerLinkFurl.path).endswith("/")
